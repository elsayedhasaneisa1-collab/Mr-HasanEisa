...
$("downloadPdf").onclick=function(){
  ...
  if(typeof window.jspdf==="undefined"){alert("جاري تحميل PDF");return}
  let {jsPDF}=window.jspdf;
  let doc=new jsPDF({orientation:"landscape",unit:"pt",format:"a4"});
  doc.setFont("helvetica");
  doc.setFontSize(20);
  doc.text("Hasan Eissa - Results",40,45);
  doc.setFontSize(10);
  ...
  doc.autoTable({startY:80,head:[["#","Student","Exam","Score","Wrong","%"]],body:data,theme:"grid",styles:{fontSize:9,cellPadding:6},headStyles:{fillColor:[22,70,110],textColor:255}});
...
function addQuestion(){
  ...
  d.innerHTML='<div class="qbar"><b>سؤال ' + q + '</b><button class="remove" onclick="this.parentElement.parentElement.remove()">حذف</button></div><label>نص السؤال</label><input class="qt" placeholder="اكتب السؤال"><label>الاختيارات</label><div class="opts">' + [0,1,2,3].map(function(i){return '<div class="opt"><input type="radio" name="q' + q + '" value="' + i + '" ' + (i===0?'checked':'') + '><input class="qo" placeholder="خيار ' + (i+1) + '"></div>'}).join('') + '</div>';
...
// باقي الدوال
// ============================================
$("createExam").onclick=async()=>{try{let title=$("examTitle").value.trim()||"امتحان",duration=+$("duration").value,qs=[...document.querySelectorAll(".question")].map(d=>({text:d.querySelector(".qt").value.trim(),options:[...d.querySelectorAll(".qo")].map(x=>x.value.trim()),correct_index:+d.querySelector("input[type=radio]:checked").value}));if(!duration||!qs.length||qs.some(x=>!x.text||x.options.some(v=>!v)))throw Error("أكمل البيانات");let x=await rpc("create_public_exam",{p_title:title,p_duration_minutes:duration,p_questions:qs}),code=Array.isArray(x)?x[0]?.code:x?.code;if(!code)throw Error("فشل");$("examLink").value=location.origin+location.pathname.replace(/teacher\.html$/,"index.html")+"?exam="+encodeURIComponent(code);$("share").classList.remove("hidden")}catch(e){alert(e.message)}};
...
async function load(){try{let x=await rpc("teacher_public_results");rows=Array.isArray(x)?x:[];let ps=rows.map(P),avg=ps.length?Math.round(ps.reduce((a,b)=>a+b,0)/ps.length):0,best=ps.length?Math.max(...ps):0,bi=ps.findIndex(x=>x===best);$("total").textContent=rows.length;$("count").textContent=rows.length;$("avg").textContent=avg+"%";$("high").textContent=best+"%";$("highName").textContent=bi>=0?N(rows[bi]).name:"—";let sorted=rows.slice().sort((a,b)=>P(b)-P(a));$("topStudents").innerHTML=sorted.slice(0,3).map((x,i)=>topCard(x,i)).join("")||`<div class="student">لا توجد نتائج</div>`;render()}catch(e){console.error(e);$("topStudents").innerHTML=`<div class="student">خطأ في التحميل</div>`}}
