...
$("downloadPdf").onclick=function(){if(!rows.length){alert("لا توجد نتائج لتنزيلها.");return}if(typeof window.jspdf==="undefined"){alert("جاري تحميل أداة PDF، حاول مرة أخرى.");return}let {jsPDF}=window.jspdf;let doc=new jsPDF({orientation:"landscape",unit:"pt",format:"a4"});doc.setFont("helvetica");doc.setFontSize(20);doc.text("Hasan Eissa - Student Results",40,45);doc.setFontSize(10);doc.text(new Date().toLocaleString("en-GB"),40,63);let data=rows.slice().sort((a,b)=>P(b)-P(a)).map((x,i)=>{let n=N(x);return [i+1,n.name,n.exam,`${n.score}/${n.total}`,`${Math.max(0,n.total-n.score)}`,`${P(x)}%`]});doc.autoTable({startY:80,head:[["#","Student","Exam","Score","Wrong","Percentage"]],body:data,theme:"grid",styles:{fontSize:9,cellPadding:6},headStyles:{fillColor:[22,70,110],textColor:255}});doc.save("Hasan-Eissa-Results.pdf")};

// دالة تصدير PDF للنسخ الاحتياطي
function exportPDF() {
  if(!rows.length) return;
  if(typeof window.jspdf === "undefined") {
    alert("جاري تحميل أداة PDF...");
    return;
  }
  ...
  doc.setFontSize(20);
  doc.text("Hasan Eissa - Student Results (BACKUP)", 40, 45);
  doc.setFontSize(10);
  doc.text("تم التصدير قبل المسح: " + new Date().toLocaleString("en-GB"), 40, 63);
  let data = rows.slice().sort((a,b) => P(b)-P(a)).map((x,i) => {
    let n = N(x);
    return [i+1, n.name, n.exam, `${n.score}/${n.total}`, `${Math.max(0,n.total-n.score)}`, `${P(x)}%`];
  });
  doc.autoTable({
    startY: 80,
...
// دالة مسح قاعدة البيانات
$("deleteAll").onclick = function() {
  let confirm1 = prompt("⚠️ تحذير: أنت على وشك حذف كل البيانات (الامتحانات والنتائج والطلاب).\nاكتب كلمة 'نعم' لتأكيد الحذف:");
  if (confirm1 !== "نعم") {
    alert("❌ تم إلغاء عملية الحذف.");
    return;
  }
  let confirm2 = confirm("⚠️ تأكيد نهائي: هل أنت متأكد 100% من حذف كل البيانات؟\nهذا الإجراء لا يمكن التراجع عنه!");
  if (!confirm2) {
    alert("❌ تم إلغاء عملية الحذف.");
    return;
  }
  let password = prompt("🔐 أدخل كلمة المرور الإضافية للمسح (رمز المسح: 998877):");
  if (password !== "998877") {
    alert("❌ كلمة المرور غير صحيحة. تم إلغاء الحذف.");
    return;
  }
  let user = prompt("👤 أدخل اسم المستخدم لتأكيد الهوية (Hasan):");
  if (user !== "Hasan") {
    alert("❌ اسم المستخدم غير صحيح. تم إلغاء الحذف.");
    return;
  }
  deleteAllData();
};
...
async function deleteAllData() {
  try {
    ...
    if (result && result.success) {
      alert("✅ تم مسح قاعدة البيانات بنجاح!\n📄 تم حفظ نسخة PDF احتياطية قبل المسح.");
      setTimeout(() => location.reload(), 1000);
    } else {
      alert("❌ حدث خطأ أثناء المسح: " + (result?.error || "خطأ غير معروف"));
    }
  } catch (e) {
    alert("❌ خطأ: " + e.message);
  } finally {
...
function addQuestion(){q++;let questionId=`q-${q}`;let d=document.createElement("div");d.className="question";d.id=questionId;d.innerHTML=`<div class="qbar"><b>السؤال ${q}</b><button class="remove" data-qid="${questionId}">حذف</button></div><label>نص السؤال</label><input class="qt" placeholder="اكتب السؤال هنا"><label>الاختيارات — اختر الإجابة الصحيحة</label><div class="opts">${[0,1,2,3].map(i=>`<div class="opt"><input type="radio" name="correct_${q}" value="${i}" ${i===0?"checked":""}><input class="qo" placeholder="الاختيار ${i+1}"></div>`).join("")}</div>`;d.querySelector(".remove").onclick=function(){let id=this.dataset.qid;let el=$(id);if(el)el.remove();};$("questions").append(d)}
